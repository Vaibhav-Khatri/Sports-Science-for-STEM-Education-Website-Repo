...
def result():
    ...
    for cat, (min_val, max_val) in categories.items():
        if cat in ['Lane Agility', 'Shuttle Run']:  # Lower is better -> higher percentile for lower values
            percentile = (historical_data[cat] > user_data[cat]).mean() * 100.0
        else:  # Higher is better -> higher percentile for higher values
            percentile = (historical_data[cat] < user_data[cat]).mean() * 100.0
        user_percentiles.append(round(float(percentile), 2))

    # Normalize percentile ranks to range [0, 1] for plotting
    normalized_values = [p / 100.0 for p in user_percentiles]

    # Prepare data for template
    combined_data = list(zip(user_data.keys(), user_percentiles))
...
def generate_spider_plot(categories, normalized_values):
    ...
    ax.fill(angles, normalized_values, color='red', alpha=0.4)
